...
def get_color(original_color, random_color=False):
    """
...
def paint_one(x, y, brushname='random', angle=-1., minrad=10, maxrad=60, random_color=False):
    """ paint one brush
    :param x:
    :param y:
    :param brushname:
    ...
    c = get_color(flower[int(y), int(x), :], random_color)
...
def putstrokes(howmany, multi_thread=True, random_color=False):
    def samplepoints():
    ...
    def pcasync(tup, random_color):
        y, x, angle = tup

        b, key = rb.get_brush(key='random')  # get a random brush
        return paint_one(x, y, brushname=key, minrad=10, maxrad=50, angle=angle, random_color=random_color)  # num of epoch

    if multi_thread:
        from thready import amap  # multithreading
        # from threading import
        point_list = samplepoints()
        return amap(pcasync, point_list, random_color)
        ...
        for idx, item in enumerate(point_list):
            ...
            res[idx] = pcasync(item, random_color)
        return res
...
# run the whole thing
def r(epoch=1, random_color=False):
    """
    ...
    for i in range(epoch):
        ...
        for k in range(loopfor):
            res = putstrokes(howmany=paranum, multi_thread=True, random_color=random_color)  # res is a map of results
